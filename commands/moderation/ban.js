...
banMember.send(`You have been banned from ${message.guild.name} for ${reason}`).then(() => {
    message.guild.members.ban(banMember, { days: botconfig.banlength, reason: reason}).catch(err => console.log(err))

    message.channel.send(`**${banMember.user.username}** has been banned, and ${botconfig.banlength} days of messages are gone!`)
